...
def main():
    ...
    for step in trange(Config.test_samples):
        ...
        with tf.Graph().as_default():
            ...
            logits = [
                tf.cast(tf.reshape(tf.nn.sigmoid(result[0][:, step[0]:step[1], :]), [1, 32, 32, 4, 1, 1]), tf.float32),
                tf.cast(tf.reshape(tf.nn.sigmoid(result[0][:, step[1]:step[2], :]), [1, 32, 32, 1, 3, 1]), tf.float32),
                tf.cast(tf.reshape(tf.nn.sigmoid(result[0][:, step[2]:step[3], :]), [1, 16, 16, 1, 3, 1]), tf.float32),
                tf.cast(tf.reshape(tf.nn.sigmoid(result[0][:, step[3]:step[4], :]), [1, 8, 8, 1, 3, 1]), tf.float32),
                tf.cast(tf.reshape(tf.nn.sigmoid(result[0][:, step[4]:step[5], :]), [1, 4, 4, 1, 3, 1]), tf.float32),
            ]
            localisations = [
                tf.cast(tf.reshape(result[1][:, step[0]:step[1], :], [1, 32, 32, 4, 1, 4]), tf.float32),
                tf.cast(tf.reshape(result[1][:, step[1]:step[2], :], [1, 32, 32, 1, 3, 4]), tf.float32),
                tf.cast(tf.reshape(result[1][:, step[2]:step[3], :], [1, 16, 16, 1, 3, 4]), tf.float32),
                tf.cast(tf.reshape(result[1][:, step[3]:step[4], :], [1, 8, 8, 1, 3, 4]), tf.float32),
                tf.cast(tf.reshape(result[1][:, step[4]:step[5], :], [1, 4, 4, 1, 3, 4]), tf.float32),
            ]
